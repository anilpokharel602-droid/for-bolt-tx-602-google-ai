        ...
        {loci.map(locus => (
          <div key={locus} className="grid grid-cols-3 items-center gap-2">
            ...
            <input
              ...
              aria-label={`${title} HLA-${locus} Allele 1`}
            ...
            <input
              ...
              aria-label={`${title} HLA-${locus} Allele 2`}
                    ...
                    <div className="space-y-3">
                        ...
                        <details className="text-sm">
                            ...
                            <ul className="mt-2 space-y-1 pl-2">
                                {Object.entries(mismatchResult.details).map(([locus, count]: [string, number]) => (
        ...
        const handler = setTimeout(() => {
            ...
            if (progress !== phase.progress || newStatus !== phase.status || JSON.stringify(updatedPhaseData) !== JSON.stringify(phase.phase5Data)) {
                setWorkflowData(prev => {
                    const newWorkflow = prev.map(p => p.id === phase.id ? { ...p, progress, status: newStatus, phase5Data: updatedPhaseData } : p);
            ...
            if (otherPhase5 && JSON.stringify(otherPhase5.phase5Data) !== JSON.stringify(updatedPhaseData)) {
                const updatedOtherWorkflow = otherWorkflow.map(p => {
                    if (p.id === 5) {
                        ...
                        return { ...p, phase5Data: updatedPhaseData, progress, status: newStatus };
                    }
                    return p;
                });
