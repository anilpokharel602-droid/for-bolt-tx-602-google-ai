    ...
    return (
        <div className="p-4 md:p-6 bg-white rounded-b-lg border border-t-0 border-gray-200 space-y-6">
            ...
            <Card>
                ...
                {activeTab === 'schedule' && <div role="tabpanel" id="panel-schedule" aria-labelledby="tab-schedule"><ScheduleView data={phaseData.operativeSchedule} onUpdate={handleDataUpdate} isLocked={isLocked} /></div>}
                {activeTab === 'checklist' && <div role="tabpanel" id="panel-checklist" aria-labelledby="tab-checklist"><ChecklistView items={patientPrep.items} onUpdate={handleChecklistUpdate} isLocked={isLocked} /></div>}
                {activeTab === 'assessment' && <div role="tabpanel" id="panel-assessment" aria-labelledby="tab-assessment"><AssessmentView data={patientPrep.assessment} onUpdate={(field, value) => handleDataUpdate(`${patient.type === PatientType.Donor ? 'donorPreparation' : 'recipientPreparation'}.assessment.${field}`, value)} isLocked={isLocked} /></div>}
            </Card>
