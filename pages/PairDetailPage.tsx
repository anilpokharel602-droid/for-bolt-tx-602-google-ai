...
const PairDetailPage: React.FC = () => {
    ...
    // Persist donor workflow changes
    useEffect(() => {
        if (donor && donorWorkflow.length > 0) {
            updateWorkflowForPatient(donor.id, donorWorkflow);
        }
    }, [donor, donorWorkflow]);

    // Persist recipient workflow changes
    useEffect(() => {
        if (recipient && recipientWorkflow.length > 0) {
            updateWorkflowForPatient(recipient.id, recipientWorkflow);
        }
    }, [recipient, recipientWorkflow]);
